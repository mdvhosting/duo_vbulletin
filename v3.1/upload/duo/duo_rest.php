...
switch($_GET['id'])
{
	...
	*/
	case "fetch_methods":
		...
		// Compute the validation hash to make sure this request is legitimate
		$validation_hash = hash_hmac("sha1", $user_id, $vbulletin->options['duosecurity_skey']);
		
		// Make sure the keys match
		if(md5($validation_hash) == md5($_GET['key']))
		{
		...
		{
			// The key wasn't valid, display an error
				...
				require(["require",
					...
					function(require, array, config, dom, kernel, ready, window, fx, registry, xhr, parser){
						ready(function(){
							...
							xhr("<?php echo $vbulletin->options['duosecurity_web_path']; ?>duo_rest.php?id=fetch_methods&userid=<?php echo $user_id; ?>&key=<?php echo hash_hmac("sha1", $user_id, $vbulletin->options['duosecurity_skey']); ?>").then(function(text)
							{
								eval_scripts(text);
				...
				*/
				function fetch_methods()
				{
					require(["dojo/request/xhr", "dojo/dom", "dojo/domReady!"],
						function(xhr, dom)
						{
							xhr("<?php echo $vbulletin->options['duosecurity_web_path']; ?>duo_rest.php?id=fetch_methods&userid=<?php echo $user_id; ?>&key=<?php echo hash_hmac("sha1", $user_id, $vbulletin->options['duosecurity_skey']); ?>", {
								method: "GET"
							}).then(function(data){
								eval_scripts(data);
						...
						<?php
							...
							if($vbulletin->options['duosecurity_showpoweredby'])
							{
								...
								<a href="http://www.duosecurity.com" target="_blank">
									<img src="<?php echo $vbulletin->options['duosecurity_image_path']; ?>duo_logo.png" width="100" height="45" />
								</a>
								<?php
							}
						?>
